...
int main() {
    ...
    // ČEKA PRVI CIKLUS ENVIRONMENT-A (2 sec stagger od temp sensora)
    ...
    while (true) {
        ...
        while (!success && retry_count < 3) {
            ...
            if (!res || res->status != 200) {
                retry_count++;
                std::cerr << "HEART Retry " << retry_count << " - Cannot fetch data.\n";
            ...
            try {
                ...
                if (!Json::parseFromStream(reader, s, &root, &errs)) {
                    retry_count++;
                    std::cerr << "HEART Retry " << retry_count << " - JSON parse error\n";
                    std::this_thread::sleep_for(std::chrono::milliseconds(300));
                    continue;
                }

                if (!root.isMember("heart_rate")) {
                    retry_count++;
                    std::cerr << "HEART Retry " << retry_count << " - No heart_rate field\n";
                    std::this_thread::sleep_for(std::chrono::milliseconds(300));
                    continue;
                }
                ...
                int result = mosquitto_publish(mosq, NULL, topic_heart_rate,
                                  payload.length(), payload.c_str(), 0, false);
                
                if (result == MOSQ_ERR_SUCCESS) {
                    std::cout << "HEART SENSOR: Published heart rate: " << heart_rate << " bpm" << std::endl;
                    success = true;
                } else {
                    retry_count++;
                    std::cerr << "HEART Retry " << retry_count << " - MQTT publish error\n";
                    std::this_thread::sleep_for(std::chrono::milliseconds(300));
                }
            }
            catch (const std::exception& e) {
                retry_count++;
                std::cerr << "HEART Retry " << retry_count << " - Exception: " << e.what() << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(300));
            }
        }

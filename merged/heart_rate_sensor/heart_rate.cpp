...
void publish_heart_rate(struct mosquitto *mosq, httplib::Client &cli) {
    while (!ctrl_c_received) {
        ...
        mosquitto_publish(mosq, NULL, topic_heart_rate, payload.length(), payload.c_str(), 0, false);

        std::cout << "Published heart rate: " << heart_rate << " bpm\n";
        std::this_thread::sleep_for(std::chrono::seconds(5));
    }
}

...
void send_discovery(int sockfd, struct sockaddr_in server_addr) {
    std::string discovery = "M-SEARCH * HTTP/1.1\r\n"
                            ...
                            "MAN: \"ssdp:discover\"\r\n"        //tip poruke, kaze da trazimo uredjaje
                            "MX: 1\r\n"                         //maksimalno vreme cekanja na odgovor
                            "ST: ssdp:all\r\n"                  //trazi sve uredjaje i servise
...
// Prima response poruku tj potvrdu od kontrolera preko upd socketa
int receive_confirmation(int sockfd, struct sockaddr_in server_addr) {
    ...
    // Ispisivanje potvrde
    std::cout << "Maschine Shutdown Relay received confirmation message from controller:" << std::endl;
...
int main() {
    ...
    if ((sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        std::cerr << "Socket creation MSR failed" << std::endl;
        return 1;
    }
